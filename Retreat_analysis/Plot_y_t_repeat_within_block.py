...
fig, axes = plt.subplots(nrows=1, ncols=blocks_n, figsize=(20, 8))
# plt.tight_layout()
...
for block in np.arange(blocks_n):
    ...
    for repeat in np.arange(5):
        ...
        repeat_df.to_pickle(plotpath+'/a.pickle')
        # alpha = np.linspace(0, 1, 6)
        # alpha = alpha[repeat + 1],  color='deepskyblue'
        sns.lineplot(data=repeat_df, x='frame', y='posy', ci=None, linewidth=1.2, err_kws={'lw': 0}, ax=ax, color=cmap(colors[repeat]), label='repeat-0'+str(repeat+1))

    ax.set_title('block-0' + str(block + 1))
...
plt.legend()
# plt.show()
figpath = os.path.join(plotpath, 'repeat_within_block.png')
